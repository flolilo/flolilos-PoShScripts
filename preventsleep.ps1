...
<#
    ...
    .PARAMETER fileToCheck
        Path to file of script that uses -fileModeEnable. 
    .PARAMETER fileModeEnable
        Value of 1 enables file-mode. Only recommended for use with other sripts.
    .PARAMETER mode
        Valid options:
    ...
    .PARAMETER userProcessCount
        Only valid when -mode = "process". Value specifies how many processes the script will look for.
    ...
    .PARAMETER timeBase
        Value of time (in seconds) that has to pass between to iterations of the script - different steps of the script will use from $timeBase/2 to $timeBase/10 as their values.
        Best practise: <= 80% of computer's shutdown- or standby-time.
    ...
    .PARAMETER shutdown
        Value of 1 will initiate shutdown of computer after finishing the script. Value of -1 (default) will ask if shutdown should be initiated or not.
...
Write-ColorOut "Flo's Preventsleep-Script v1.4 /" -ForegroundColor Cyan -NoNewline
Write-ColorOut "/ Flos Schlaf-Verhinder-Skript v1.4" -ForegroundColor Yellow
